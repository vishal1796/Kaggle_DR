...
def scaleRadius(img, scale):
    """ To scale the given image based on a scale value for the radius """
...
def preprocess(folder='sample', scales=[300], save_path_same=True, target_size=(0,0)):
    """
    To preprocess images from a give folder and save them
    Args:
    ...
    handlers = [logging.FileHandler(datetime.now().strftime("%Y-%m-%d_%H-%M-%S-{file}.log")), logging.StreamHandler()]
